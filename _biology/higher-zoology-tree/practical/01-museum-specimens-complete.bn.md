---
layout: single
author_profile: true
author: "MD. Yusuf"
sidebar:
  nav: "zoology_practical_213106"
title: "Practical 01 — Museum Specimens: Complete NU 48/48"
excerpt: "NU Honours Zoology Practical-I museum spotting: 48/48 unique syllabus labels with classification, figure cues and diagnostic characters."
permalink: /biology/higher-zoology-tree/practical/museum-specimens/
categories:
  - Biology
  - Higher Zoology
  - Zoology Practical
  - NU Honours Zoology
tags:
  - Zoology Practical-I
  - NU Honours 1st Year
course_id: zoology-practical-213106
course_role: practical-lecture
page_id: zoology-practical-museum-complete
language: bn
lang: bn
locale: bn-BD
toc: true
toc_sticky: true
classes: wide zoology-practical-page
related: true
status: Draft-Ready
published: true
last_modified_at: "2026-09-22"
curriculum_tracks:
  - NU Honours Zoology
nu_alignment: "National University B.Sc. Honours 1st Year Zoology — Zoology Practical-I (213106)"
concept_level: Practical / Laboratory
difficulty: Intermediate
time_min: 240
---

<link rel="stylesheet" href="{{ '/assets/css/zoology-practical.css' | relative_url }}">
<script src="{{ '/assets/js/zoology-practical.js' | relative_url }}" defer></script>

# Study of Museum Specimens — Complete NU Coverage

## Classification, Figure Cues & Identifying Characters

**মূল Lecture:** বাংলা  
**Technical Terms:** English  
**Coverage:** **48/48 unique syllabus labels**

NU list-এ `Echinus` দুইবার printed; তাই 49 printed entries থেকে **48 unique labels** পাওয়া যায়। এই page সব 48 unique label cover করে।

## Museum Practical Answer Pattern

**Specimen → Classification → 3–5 visible identifying characters → one diagnostic character → identification conclusion**

> Habitat বা disease information supporting evidence হতে পারে; preserved specimen spotting-এ প্রথম evidence হবে **visible morphology**।

## Figure Policy

প্রতিটি specimen-এর জন্য local asset path:
`/assets/biology/higher-zoology-tree/practical/museum-specimens/<slug>.svg`

Figure-এ minimum:
1. silhouette / orientation;
2. 2–5 diagnostic labels;
3. scale বা “not to scale” note;
4. source/licence metadata যদি external reference থেকে adapted হয়।


---

## 1. Sycon — *Sycon ciliatum*

**Common name:** Calcareous sponge / purse sponge  
**Figure:** `museum-specimens/sycon.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Porifera |
| Class | Calcarea |
| Subclass | Calcaronea |
| Order | Leucosolenida |
| Family | Sycettidae |
| Genus | *Sycon* |
| Representative | *Sycon ciliatum* |

### শনাক্তকারী বৈশিষ্ট্য

- দেহ ছোট, vase-shaped বা cylindrical এবং substratum-এর সঙ্গে attached।
- শীর্ষে terminal **osculum** থাকে।
- Osculum-এর চারপাশে long calcareous **spicules**-এর fringe থাকে।
- Body wall-এ অসংখ্য **ostia** থাকে।
- Canal system **syconoid type**; skeleton calcareous spicules-নির্মিত।

**Diagnostic character:** Porous body + terminal osculum + calcareous spicules.

### Practical identification

> অসংখ্য ostia, terminal osculum এবং calcareous spicules-এর কারণে specimen-টি *Sycon*।

**Taxonomy / syllabus note:** Family-level usage sourceভেদে পরিবর্তিত হতে পারে; exam-এ departmental classification preserve করবে।


---

## 2. Physalia — *Physalia physalis*

**Common name:** Portuguese man-of-war  
**Figure:** `museum-specimens/physalia.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Cnidaria |
| Class | Hydrozoa |
| Clade | Hydroidolina |
| Order | Siphonophorae |
| Suborder | Cystonectae |
| Family | Physaliidae |
| Genus | *Physalia* |
| Species | *P. physalis* |

### শনাক্তকারী বৈশিষ্ট্য

- এটি একক medusa নয়; একটি **polymorphic colonial hydrozoan**।
- উপরের gas-filled **pneumatophore** নীল-বেগুনি float তৈরি করে।
- Float-এ crest/sail-like ridge থাকে।
- নিচে food-capture ও defence-এর জন্য বহু zooid ও অত্যন্ত দীর্ঘ **tentacles** থাকে।
- Tentacles-এ শক্তিশালী nematocysts থাকে।

**Diagnostic character:** Large gas-filled pneumatophore with long trailing tentacles in a siphonophore colony.

### Practical identification

> Gas-filled pneumatophore এবং long tentacular zooids থাকার কারণে specimen-টি *Physalia*।


---

## 3. Metridium — *Metridium senile*

**Common name:** Frilled sea anemone  
**Figure:** `museum-specimens/metridium.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Cnidaria |
| Class | Anthozoa |
| Subclass | Hexacorallia |
| Order | Actiniaria |
| Family | Metridiidae |
| Genus | *Metridium* |
| Representative | *M. senile* |

### শনাক্তকারী বৈশিষ্ট্য

- Body cylindrical **polyp**; medusa stage absent।
- Broad **pedal disc** দ্বারা substratum-এ attached।
- Oral disc-এর কেন্দ্রে mouth এবং চারদিকে অসংখ্য slender tentacles।
- Body wall soft; hard skeleton absent।
- Oral region expanded হলে characteristic frilled appearance দেখা যায়।

**Diagnostic character:** Large soft sea-anemone polyp with broad pedal disc and numerous fine tentacles.

### Practical identification

> Broad pedal disc, columnar body এবং numerous oral tentacles-এর কারণে specimen-টি *Metridium*।


---

## 4. Adamsia — *Adamsia palliata*

**Common name:** Cloak anemone / hermit-crab anemone  
**Figure:** `museum-specimens/adamsia.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Cnidaria |
| Class | Anthozoa |
| Subclass | Hexacorallia |
| Order | Actiniaria |
| Family | Hormathiidae |
| Genus | *Adamsia* |
| Species | *A. palliata* |

### শনাক্তকারী বৈশিষ্ট্য

- দেহ polyp form; medusa stage absent।
- Oral disc-এ central mouth ও বহু tentacles।
- Tentacles-এ cnidocytes/nematocysts থাকে।
- Basal **pedal disc** বিস্তৃত।
- Gastropod shell-এ hermit crab-এর সঙ্গে association-এ পাওয়া যায়।

**Diagnostic character:** Sea-anemone polyp attached to/associated with a gastropod shell and hermit crab.

### Practical identification

> Tentacled actiniarian polyp এবং hermit-crab association-এর কারণে specimen-টি *Adamsia*।


---

## 5. Arenicola — *Arenicola marina*

**Common name:** Lugworm  
**Figure:** `museum-specimens/arenicola.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Traditional class | Polychaeta |
| Clade | Sedentaria |
| Group | Scolecida |
| Family | Arenicolidae |
| Genus | *Arenicola* |
| Representative | *A. marina* |

### শনাক্তকারী বৈশিষ্ট্য

- দেহ মোটা, elongated ও distinctly **segmented**।
- Anterior region তুলনামূলক swollen; posterior tail region narrow।
- Mid-body segments-এ paired, bushy **branchiae/gills** থাকে।
- Parapodia reduced; chaetae present।
- Marine sand/mud burrow-এ বসবাসকারী lugworm।

**Diagnostic character:** Robust segmented polychaete with conspicuous paired bushy gills on mid-body.

### Practical identification

> Segmented body এবং mid-body-তে paired bushy gills থাকার কারণে specimen-টি *Arenicola*।


---

## 6. Tubifex — *Tubifex tubifex*

**Common name:** Sludge worm  
**Figure:** `museum-specimens/tubifex.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Class | Clitellata |
| Traditional group | Oligochaeta |
| Order | Tubificida |
| Family | Naididae |
| Subfamily | Tubificinae |
| Genus | *Tubifex* |
| Species | *T. tubifex* |

### শনাক্তকারী বৈশিষ্ট্য

- দেহ অত্যন্ত slender, elongated ও segmented।
- Living specimen সাধারণত reddish।
- Segments-এ chaetae থাকে; parapodia absent।
- Organic-rich freshwater mud/sludge-এ পাওয়া যায়।
- True metameric segmentation স্পষ্ট।

**Diagnostic character:** Very slender aquatic annelid with true segmentation and chaetae.

### Practical identification

> সরু segmented aquatic body এবং chaetae-এর কারণে specimen-টি *Tubifex*।


---

## 7. Metaphere — “Metaphere” — syllabus label; probable *Metaphire* sp.

**Common name:** Earthworm-type megascolecid  
**Figure:** `museum-specimens/metaphere.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Class | Clitellata |
| Clade | Crassiclitellata |
| Family | Megascolecidae |
| Probable genus | *Metaphire* |
| Syllabus label | Metaphere |

### শনাক্তকারী বৈশিষ্ট্য

- দেহ cylindrical ও metamerically segmented।
- Mature specimen-এ clitellum থাকে।
- Megascolecid earthworm-এ বহু chaetae per segment থাকতে পারে।
- External genital pores ও spermathecal pores specimen identification-এ গুরুত্বপূর্ণ।
- Jar label এবং departmental key species-level identification-এর জন্য অপরিহার্য।

**Diagnostic character:** Earthworm morphology with megascolecid/perichaetine pattern, but syllabus spelling is taxonomically ambiguous.

### Practical identification

> Syllabus label “Metaphere” অনুযায়ী specimen-টি earthworm-type megascolecid; final genus spelling jar label ও departmental key দিয়ে verify করতে হবে।

**Taxonomy / syllabus note:** এটিকে automaticভাবে *Metaphire posthuma* ধরে নেওয়া হবে না, কারণ একই NU list-এ “Pheretima” আলাদা entry।


---

## 8. Tomopteris — *Tomopteris* sp.

**Common name:** Pelagic polychaete  
**Figure:** `museum-specimens/tomopteris.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Traditional class | Polychaeta |
| Clade | Errantia |
| Order | Phyllodocida |
| Family | Tomopteridae |
| Genus | *Tomopteris* |

### শনাক্তকারী বৈশিষ্ট্য

- Body slender, delicate ও প্রায় transparent।
- Pelagic life-এর জন্য lateral **paddle-like parapodia** অত্যন্ত developed।
- Head-এ elongated tentacular structures/cirri থাকে।
- Typical chaetae largely absent in adult Tomopteris।
- Parapodia sequentialভাবে body-এর দুই পাশে prominent।

**Diagnostic character:** Transparent pelagic polychaete with conspicuous paddle-like parapodia.

### Practical identification

> Transparent body ও enlarged paddle-like parapodia থাকার কারণে specimen-টি *Tomopteris*।


---

## 9. Amphitrite — *Amphitrite ornata*

**Common name:** Terebellid / spaghetti worm  
**Figure:** `museum-specimens/amphitrite.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Traditional class | Polychaeta |
| Clade | Sedentaria |
| Order | Terebellida |
| Family | Terebellidae |
| Genus | *Amphitrite* |
| Representative | *A. ornata* |

### শনাক্তকারী বৈশিষ্ট্য

- Anterior end থেকে বহু long, thread-like **feeding tentacles** বের হয়।
- Anterior segments-এ branched **gills** থাকে।
- Body segmented; chaetae-bearing parapodial structures comparatively reduced।
- Tube-dwelling sedentary polychaete।
- Posterior body elongated ও more uniform।

**Diagnostic character:** Numerous long feeding tentacles plus anterior branched gills.

### Practical identification

> Head region-এ numerous feeding tentacles এবং anterior branched gills থাকার কারণে specimen-টি *Amphitrite*।


---

## 10. Lumbricus — *Lumbricus terrestris*

**Common name:** Common earthworm  
**Figure:** `museum-specimens/lumbricus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Class | Clitellata |
| Clade | Crassiclitellata |
| Family | Lumbricidae |
| Genus | *Lumbricus* |
| Species | *L. terrestris* |

### শনাক্তকারী বৈশিষ্ট্য

- দেহ cylindrical ও externally segmented।
- Typical segment-এ সাধারণত 8 chaetae থাকে।
- Mature specimen-এ conspicuous clitellum।
- *L. terrestris*-এ clitellum প্রায় segments 32–37।
- Dorsal side darker, ventral side lighter হতে পারে।

**Diagnostic character:** Earthworm with few paired chaetae and relatively posterior clitellum.

### Practical identification

> Segmented body, paired chaetae এবং posteriorly placed clitellum-এর কারণে specimen-টি *Lumbricus*।


---

## 11. Ancylostoma — *Ancylostoma duodenale*

**Common name:** Old World hookworm  
**Figure:** `museum-specimens/ancylostoma.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Chromadorea |
| Order | Rhabditida |
| Superfamily | Strongyloidea |
| Family | Ancylostomatidae |
| Genus | *Ancylostoma* |
| Species | *A. duodenale* |

### শনাক্তকারী বৈশিষ্ট্য

- Body slender, cylindrical ও unsegmented।
- Anterior end characteristicভাবে hook-like bent।
- Buccal capsule-এ teeth থাকে।
- Sexes separate।
- Male posterior end-এ copulatory bursa থাকে।

**Diagnostic character:** Hooked anterior end + toothed buccal capsule; male with copulatory bursa.

### Practical identification

> Hook-shaped anterior end এবং copulatory bursa-এর কারণে specimen-টি *Ancylostoma*।


---

## 12. Enterobius — *Enterobius vermicularis*

**Common name:** Pinworm  
**Figure:** `museum-specimens/enterobius.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Chromadorea |
| Order | Rhabditida |
| Superfamily | Oxyuroidea |
| Family | Oxyuridae |
| Genus | *Enterobius* |
| Species | *E. vermicularis* |

### শনাক্তকারী বৈশিষ্ট্য

- Small, slender, white thread-like worm।
- Anterior end-এ **cephalic alae** থাকে।
- Posterior oesophagus-এ prominent bulb।
- Female-এর tail long এবং pointed; male much smaller ও posteriorly curved।
- Egg asymmetrical; এক পাশ flattened।

**Diagnostic character:** Cephalic alae + long pointed female tail.

### Practical identification

> Cephalic alae এবং female-এর pointed tail থাকার কারণে specimen-টি *Enterobius vermicularis*।


---

## 13. Wuchereria — *Wuchereria bancrofti*

**Common name:** Filarial worm  
**Figure:** `museum-specimens/wuchereria.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Chromadorea |
| Order | Rhabditida |
| Superfamily | Filarioidea |
| Family | Onchocercidae |
| Genus | *Wuchereria* |
| Species | *W. bancrofti* |

### শনাক্তকারী বৈশিষ্ট্য

- Adult body extremely slender, long ও thread-like।
- Sexes separate; female larger।
- Adults lymphatic vessels-এ থাকে।
- Microfilaria **sheathed**।
- Stained microfilaria-তে nuclei tail-tip পর্যন্ত পৌঁছায় না।

**Diagnostic character:** Sheathed microfilaria with terminal tail free of nuclei.

### Practical identification

> Thread-like filarial form এবং microfilarial tail-tip-এ nuclei অনুপস্থিত থাকার কারণে specimen-টি *Wuchereria bancrofti*।

**Taxonomy / syllabus note:** Adult museum specimen এবং blood-smear microfilaria-র diagnostic characters আলাদা।


---

## 14. Trichinella — *Trichinella spiralis*

**Common name:** Trichina worm  
**Figure:** `museum-specimens/trichinella.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Enoplea |
| Clade | Dorylaimia |
| Order | Trichinellida |
| Family | Trichinellidae |
| Genus | *Trichinella* |
| Species | *T. spiralis* |

### শনাক্তকারী বৈশিষ্ট্য

- Adult worms minute ও slender।
- Female viviparous/larviparous এবং male-এর চেয়ে বড়।
- Larva striated muscle fibre-এর মধ্যে characteristic **coiled** form নেয়।
- Muscle-stage larva nurse-cell complex-এর মধ্যে থাকতে পারে।
- Museum/slide spotting-এ coiled encysted larva সবচেয়ে useful clue।

**Diagnostic character:** Coiled larva embedded in striated skeletal muscle.

### Practical identification

> Striated muscle-এর মধ্যে coiled larval form থাকার কারণে specimen/slide-টি *Trichinella spiralis*।


---

## 15. Dracunculus — *Dracunculus medinensis*

**Common name:** Guinea worm  
**Figure:** `museum-specimens/dracunculus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Chromadorea |
| Order | Rhabditida |
| Superfamily | Dracunculoidea |
| Family | Dracunculidae |
| Genus | *Dracunculus* |
| Species | *D. medinensis* |

### শনাক্তকারী বৈশিষ্ট্য

- Adult female exceptionally long, thin, thread-like—প্রায় metre-scale হতে পারে।
- Male female-এর তুলনায় খুব ছোট।
- Gravid female subcutaneous tissue-এ অবস্থান করে।
- Uterus larvae-তে পূর্ণ থাকে।
- Body smooth, cylindrical ও unsegmented।

**Diagnostic character:** Exceptionally long, thread-like gravid female nematode.

### Practical identification

> অস্বাভাবিকভাবে দীর্ঘ thread-like gravid female body-এর কারণে specimen-টি *Dracunculus medinensis*।


---

## 16. Loa — *Loa loa*

**Common name:** African eye worm  
**Figure:** `museum-specimens/loa.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Chromadorea |
| Order | Rhabditida |
| Superfamily | Filarioidea |
| Family | Onchocercidae |
| Genus | *Loa* |
| Species | *L. loa* |

### শনাক্তকারী বৈশিষ্ট্য

- Adult slender filarial worm; subcutaneous tissue-এ থাকে।
- Cuticle-এ irregular **bosses** থাকতে পারে।
- Microfilaria sheathed।
- Microfilarial tail tapered এবং nuclei **tail tip পর্যন্ত** extend করে।
- Microfilariae-এর diurnal periodicity diagnostic context-এ গুরুত্বপূর্ণ।

**Diagnostic character:** Sheathed microfilaria with nuclei continuing to the tip of the tail.

### Practical identification

> Sheathed microfilaria এবং tail-tip পর্যন্ত nuclei থাকার কারণে এটি *Loa loa*।


---

## 17. Trichuris — *Trichuris trichiura*

**Common name:** Whipworm  
**Figure:** `museum-specimens/trichuris.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Nematoda |
| Class | Enoplea |
| Clade | Dorylaimia |
| Order | Trichinellida |
| Family | Trichuridae |
| Genus | *Trichuris* |
| Species | *T. trichiura* |

### শনাক্তকারী বৈশিষ্ট্য

- Anterior প্রায় two-thirds extremely thin ও whip-like।
- Posterior অংশ thicker—handle-এর মতো।
- Male-এর posterior end coiled; female comparatively straight।
- Egg barrel-shaped with **bipolar plugs**।
- Adult cecum/large intestine-এ থাকে।

**Diagnostic character:** Whip-like thin anterior body + thick posterior; egg with bipolar plugs.

### Practical identification

> Whip-like anterior end ও thick posterior region-এর কারণে specimen-টি *Trichuris trichiura*।


---

## 18. Hirudo — *Hirudo medicinalis*

**Common name:** Medicinal leech  
**Figure:** `museum-specimens/hirudo.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Class | Clitellata |
| Subclass | Hirudinea |
| Order | Hirudinida |
| Family | Hirudinidae |
| Genus | *Hirudo* |
| Species | *H. medicinalis* |

### শনাক্তকারী বৈশিষ্ট্য

- Body dorso-ventrally flattened।
- Anterior ও posterior দুই প্রান্তে suckers।
- External annuli numerous।
- Chaetae ও parapodia absent।
- Jawed medicinal leech-এ mouth anterior sucker-এর মধ্যে থাকে।

**Diagnostic character:** Flattened annelid with two suckers and no chaetae.

### Practical identification

> Dorso-ventrally flattened body, paired end suckers এবং chaetae-এর অনুপস্থিতির কারণে specimen-টি *Hirudo*।


---

## 19. Piscicola — *Piscicola geometra*

**Common name:** Fish leech  
**Figure:** `museum-specimens/piscicola.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Class | Clitellata |
| Subclass | Hirudinea |
| Order | Hirudinida |
| Group | Oceanobdelliformes |
| Family | Piscicolidae |
| Genus | *Piscicola* |
| Representative | *P. geometra* |

### শনাক্তকারী বৈশিষ্ট্য

- Body elongated, narrow এবং annulated।
- Small anterior sucker এবং large posterior sucker থাকে।
- Parasitic attachment-এর জন্য posterior sucker খুব conspicuous।
- Chaetae absent।
- Fish-এর external surface/gills-এ ectoparasitic।

**Diagnostic character:** Slender fish leech with markedly developed posterior sucker.

### Practical identification

> Elongated annulated body ও conspicuous posterior sucker থাকার কারণে specimen-টি *Piscicola*।


---

## 20. Pheretima — *Pheretima* — traditional syllabus label

**Common name:** Earthworm  
**Figure:** `museum-specimens/pheretima.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Annelida |
| Class | Clitellata |
| Clade | Crassiclitellata |
| Family | Megascolecidae |
| Genus | *Pheretima* sensu syllabus |
| Traditional textbook species | *Pheretima posthuma* |

### শনাক্তকারী বৈশিষ্ট্য

- Body long, cylindrical ও metamerically segmented।
- Traditional *P. posthuma* practical-এ perichaetine setae described হয়।
- Clitellum traditionally segments 14–16-এ shown হয়।
- Prostomium epilobous type হিসেবে taught হয়।
- External genital/spermathecal pores useful practical landmarks।

**Diagnostic character:** Traditional perichaetine earthworm with clitellum on segments 14–16.

### Practical identification

> Perichaetine setae এবং segments 14–16-এর clitellum-এর ভিত্তিতে traditional practical specimen-টি *Pheretima* হিসেবে শনাক্ত করা হয়।

**Taxonomy / syllabus note:** Modern taxonomy-তে textbook *Pheretima posthuma* সাধারণত *Metaphire posthuma* নামে treated। কিন্তু syllabus-এ Pheretima আলাদা label, তাই exam-facing label preserve করা হয়েছে।


---

## 21. Sipunculus — *Sipunculus nudus*

**Common name:** Peanut worm  
**Figure:** `museum-specimens/sipunculus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Modern phylum placement | Annelida |
| Traditional phylum | Sipuncula |
| Class | Sipunculidea |
| Order | Golfingiida |
| Family | Sipunculidae |
| Genus | *Sipunculus* |
| Representative | *S. nudus* |

### শনাক্তকারী বৈশিষ্ট্য

- Body unsegmented, cylindrical/peanut-shaped।
- Anterior **introvert** retractile।
- Mouth introvert-এর tip-এ এবং tentacles দ্বারা surrounded।
- Anus anterior trunk-এ, introvert base-এর কাছে dorsally opens।
- Chaetae absent; true external segmentation নেই।

**Diagnostic character:** Unsegmented peanut-shaped trunk with retractile introvert.

### Practical identification

> Retractile introvert এবং unsegmented peanut-like body-এর কারণে specimen-টি *Sipunculus*।

**Taxonomy / syllabus note:** Traditional courses Sipuncula-কে separate phylum হিসেবে শেখায়; molecular phylogeny এটিকে Annelida-এর মধ্যে রাখে।


---

## 22. Fasciola — *Fasciola hepatica*

**Common name:** Liver fluke  
**Figure:** `museum-specimens/fasciola.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Platyhelminthes |
| Class | Trematoda |
| Subclass | Digenea |
| Order | Plagiorchiida |
| Family | Fasciolidae |
| Genus | *Fasciola* |
| Species | *F. hepatica* |

### শনাক্তকারী বৈশিষ্ট্য

- Body broad leaf-shaped ও flattened।
- Anterior end-এ cephalic cone ও shoulders।
- Oral sucker ও ventral sucker present।
- Adult hermaphrodite।
- Branched intestinal caeca ও reproductive organs body fill করে।

**Diagnostic character:** Leaf-shaped body with cephalic cone and two suckers.

### Practical identification

> Leaf-shaped body, cephalic cone এবং oral/ventral sucker-এর কারণে specimen-টি *Fasciola hepatica*।


---

## 23. Schistosoma — *Schistosoma* sp. (e.g., *S. mansoni*)

**Common name:** Blood fluke  
**Figure:** `museum-specimens/schistosoma.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Platyhelminthes |
| Class | Trematoda |
| Subclass | Digenea |
| Order | Strigeidida |
| Family | Schistosomatidae |
| Genus | *Schistosoma* |

### শনাক্তকারী বৈশিষ্ট্য

- Sexes separate—trematodes-এর মধ্যে distinctive।
- Male short, thick; female long, slender।
- Male-এর ventral side-এ **gynecophoral canal**।
- Female সাধারণত canal-এর মধ্যে held থাকে।
- Adults blood vessels-এ live করে।

**Diagnostic character:** Dioecious blood fluke with male gynecophoral canal.

### Practical identification

> Separate sexes এবং male-এর gynecophoral canal থাকার কারণে specimen-টি *Schistosoma*।


---

## 24. Convoluta — *Convoluta convoluta*

**Common name:** Acoel flatworm  
**Figure:** `museum-specimens/convoluta.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Xenacoelomorpha |
| Subphylum | Acoelomorpha |
| Order/Class-level group | Acoela |
| Family | Convolutidae |
| Genus | *Convoluta* |
| Representative | *C. convoluta* |

### শনাক্তকারী বৈশিষ্ট্য

- Body minute, soft ও dorsoventrally flattened।
- Body surface ciliated; locomotion gliding type।
- True through-gut absent; digestive tissue simplified।
- Anterior sensory region/statocyst present।
- Traditional zoology books একে Turbellaria/Platyhelminthes-এর সঙ্গে আলোচনা করতে পারে।

**Diagnostic character:** Minute ciliated acoel with flattened body and no conventional gut.

### Practical identification

> Ciliated flattened acoel body-এর কারণে specimen-টি *Convoluta*।

**Taxonomy / syllabus note:** Modern phylogeny *Convoluta*-কে Platyhelminthes নয়, Xenacoelomorpha/Acoela-তে রাখে। Exam answer-এ syllabus framework আলাদা করে লিখবে।


---

## 25. Diphyllobothrium — Traditional *Diphyllobothrium latum* = modern *Dibothriocephalus latus*

**Common name:** Broad fish tapeworm  
**Figure:** `museum-specimens/diphyllobothrium.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Platyhelminthes |
| Class | Cestoda |
| Subclass | Eucestoda |
| Order | Diphyllobothriidea |
| Family | Diphyllobothriidae |
| Modern genus | *Dibothriocephalus* |
| Species | *D. latus* |

### শনাক্তকারী বৈশিষ্ট্য

- Body long, ribbon-like ও segmented into proglottids।
- Scolex elongate এবং **bothria** (grooves) থাকে; four cup-like suckers নেই।
- Proglottids **broader than long**।
- Gravid proglottid-এ central rosette-like uterus।
- Neck থেকে strobila develops।

**Diagnostic character:** Scolex with two bothria + broad proglottids + rosette-shaped uterus.

### Practical identification

> Bothria-bearing scolex এবং broader-than-long proglottids-এর কারণে specimen-টি traditional *Diphyllobothrium latum* / modern *Dibothriocephalus latus*।


---

## 26. Hymenolepis — Traditional *Hymenolepis nana* = current *Rodentolepis nana* in NCBI

**Common name:** Dwarf tapeworm  
**Figure:** `museum-specimens/hymenolepis.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Platyhelminthes |
| Class | Cestoda |
| Subclass | Eucestoda |
| Order | Cyclophyllidea |
| Family | Hymenolepididae |
| Current NCBI genus | *Rodentolepis* |
| Species | *R. nana* |

### শনাক্তকারী বৈশিষ্ট্য

- Adult cestode small/dwarf।
- Scolex globular with **4 suckers**।
- Retractile rostellum-এ single row of hooks।
- Strobila short with many small proglottids।
- Egg-এ polar thickenings থেকে **polar filaments** বের হয়।

**Diagnostic character:** Four-suckered scolex with hooked rostellum; eggs with polar filaments.

### Practical identification

> Hooked rostellum, four suckers এবং polar-filament egg-এর কারণে specimen-টি syllabus *Hymenolepis nana*।

**Taxonomy / syllabus note:** CDC এখনও Hymenolepis nana নাম ব্যবহার করে; NCBI homotypic synonym হিসেবে *Rodentolepis nana* current name দেখায়।


---

## 27. Lingula — *Lingula anatina*

**Common name:** Tongue shell  
**Figure:** `museum-specimens/lingula.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Brachiopoda |
| Subphylum | Linguliformea |
| Class | Lingulata |
| Order | Lingulida |
| Family | Lingulidae |
| Genus | *Lingula* |
| Representative | *L. anatina* |

### শনাক্তকারী বৈশিষ্ট্য

- Shell two valves কিন্তু valves **dorsal and ventral**, bivalve mollusc-এর left-right নয়।
- Shell elongated, tongue-shaped এবং comparatively thin।
- Long muscular **pedicle** posteriorly emerges।
- Valves mainly organophosphatic/chitinous composition।
- Lophophore internal feeding organ।

**Diagnostic character:** Tongue-shaped brachiopod shell with long pedicle and dorsoventral valves.

### Practical identification

> Dorsoventral bivalved shell ও long pedicle-এর কারণে specimen-টি *Lingula*।


---

## 28. Dentalium — *Dentalium* sp.

**Common name:** Tusk shell  
**Figure:** `museum-specimens/dentalium.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Scaphopoda |
| Order | Dentaliida |
| Family | Dentaliidae |
| Genus | *Dentalium* |

### শনাক্তকারী বৈশিষ্ট্য

- Shell long, tubular, slightly curved ও tusk-like।
- Shell **open at both ends**।
- Anterior wider aperture থেকে foot/head region বের হয়।
- Posterior aperture narrow।
- Feeding captacula thread-like structures হিসেবে ব্যবহৃত হয়।

**Diagnostic character:** Curved tusk-shaped tubular shell open at both ends.

### Practical identification

> দুই প্রান্তে open, curved tusk-like shell থাকার কারণে specimen-টি *Dentalium*।


---

## 29. Chiton — *Chiton* sp.

**Common name:** Chiton / coat-of-mail shell  
**Figure:** `museum-specimens/chiton.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Polyplacophora |
| Clade | Neoloricata |
| Order | Chitonida |
| Family | Chitonidae |
| Genus | *Chiton* |

### শনাক্তকারী বৈশিষ্ট্য

- Body oval ও dorsoventrally flattened।
- Dorsal surface-এ **8 overlapping shell plates** থাকে।
- Plates-এর চারদিকে muscular **girdle**।
- Ventral side-এ broad creeping foot।
- Head poorly differentiated; multiple gills pallial groove-এ।

**Diagnostic character:** Eight dorsal shell plates surrounded by a broad girdle.

### Practical identification

> Eight dorsal plates ও broad girdle থাকার কারণে specimen-টি *Chiton*।


---

## 30. Neopilina — *Neopilina galatheae*

**Common name:** Living monoplacophoran  
**Figure:** `museum-specimens/neopilina.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Monoplacophora |
| Order | Neopilinida |
| Family | Neopilinidae |
| Genus | *Neopilina* |
| Species | *N. galatheae* |

### শনাক্তকারী বৈশিষ্ট্য

- Shell single, cap-like ও bilaterally symmetrical।
- Broad ventral foot থাকে।
- Several organs—gills, nephridia, retractor muscles—**serially repeated**।
- Head reduced; shell limpet-like।
- Deep-sea mollusc হিসেবে পরিচিত।

**Diagnostic character:** Single cap-like shell plus serial repetition of paired internal structures.

### Practical identification

> Cap-like univalve shell এবং serially repeated organs-এর কারণে specimen-টি *Neopilina*।


---

## 31. Pila — *Pila globosa*

**Common name:** Indian apple snail  
**Figure:** `museum-specimens/pila.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Gastropoda |
| Subclass | Caenogastropoda |
| Order/group | Architaenioglossa |
| Family | Ampullariidae |
| Genus | *Pila* |
| Species | *P. globosa* |

### শনাক্তকারী বৈশিষ্ট্য

- Shell globose ও spirally coiled।
- Usually dextral।
- Large body whorl ও short spire।
- Broad aperture **operculum** দিয়ে close করা যায়।
- Living animal-এ muscular foot এবং tentacled head।

**Diagnostic character:** Globose dextral shell with broad operculate aperture.

### Practical identification

> Globose coiled shell, broad aperture এবং operculum থাকার কারণে specimen-টি *Pila*।


---

## 32. Mytilus — *Mytilus edulis*

**Common name:** Blue mussel  
**Figure:** `museum-specimens/mytilus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Bivalvia |
| Subclass | Autobranchia |
| Order | Mytilida |
| Family | Mytilidae |
| Genus | *Mytilus* |
| Representative | *M. edulis* |

### শনাক্তকারী বৈশিষ্ট্য

- Shell elongated, wedge-shaped ও equivalve but inequilateral।
- Umbo near anterior end।
- External shell সাধারণত dark blue/blackish।
- Attachment-এর জন্য **byssus threads** তৈরি করে।
- Anterior adductor comparatively small; posterior adductor larger।

**Diagnostic character:** Wedge-shaped mussel shell with anterior umbo and byssal attachment.

### Practical identification

> Elongated wedge-shaped shell এবং byssal attachment-এর কারণে specimen-টি *Mytilus*।


---

## 33. Unio — *Unio pictorum*

**Common name:** Freshwater mussel / painter's mussel  
**Figure:** `museum-specimens/unio.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Bivalvia |
| Subclass | Autobranchia |
| Clade | Palaeoheterodonta |
| Order | Unionida |
| Family | Unionidae |
| Genus | *Unio* |
| Representative | *U. pictorum* |

### শনাক্তকারী বৈশিষ্ট্য

- Freshwater bivalve with two laterally placed valves।
- Dorsal margin-এ umbo ও hinge apparatus।
- Shell interior nacreous।
- Adult-এ broad, hatchet-shaped muscular foot।
- Hinge-এ cardinal/lateral teeth থাকতে পারে; byssus adult stage-এ absent।

**Diagnostic character:** Freshwater unionid shell with umbo, hinge teeth and nacreous inner surface.

### Practical identification

> Freshwater bivalved shell, dorsal umbo ও unionid hinge characters-এর কারণে specimen-টি *Unio*।


---

## 34. Nautilus — *Nautilus pompilius*

**Common name:** Chambered nautilus  
**Figure:** `museum-specimens/nautilus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Cephalopoda |
| Subclass | Nautiloidea |
| Order | Nautilida |
| Family | Nautilidae |
| Genus | *Nautilus* |
| Representative | *N. pompilius* |

### শনাক্তকারী বৈশিষ্ট্য

- Large external **planispirally coiled chambered shell**।
- Septa shell-কে chambers-এ divide করে; siphuncle chambers traverse করে।
- Head-এ numerous tentacles/cirri, typical octopus suckers absent।
- Large hood shell opening close করতে সাহায্য করে।
- Living animal outermost chamber-এ থাকে।

**Diagnostic character:** External chambered coiled shell with many suckerless tentacles.

### Practical identification

> External chambered shell এবং numerous suckerless tentacles-এর কারণে specimen-টি *Nautilus*।


---

## 35. Sepia — *Sepia officinalis*

**Common name:** Cuttlefish  
**Figure:** `museum-specimens/sepia.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Cephalopoda |
| Subclass | Coleoidea |
| Clade | Decapodiformes |
| Order | Sepiida |
| Family | Sepiidae |
| Genus | *Sepia* |
| Representative | *S. officinalis* |

### শনাক্তকারী বৈশিষ্ট্য

- Body oval/flattened mantle with lateral fin running along much of mantle margin।
- Eight short arms + **two long retractile tentacles**।
- Internal calcareous **cuttlebone** present।
- Large eyes; funnel and ink sac present।
- Tentacular clubs bear suckers।

**Diagnostic character:** Cuttlebone + 8 arms and 2 tentacles + broad lateral fin.

### Practical identification

> Internal cuttlebone, ten appendages এবং mantle fin-এর কারণে specimen-টি *Sepia*।


---

## 36. Loligo — *Loligo vulgaris*

**Common name:** Squid  
**Figure:** `museum-specimens/loligo.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Cephalopoda |
| Subclass | Coleoidea |
| Clade | Decapodiformes |
| Order | Myopsida |
| Family | Loliginidae |
| Genus | *Loligo* |
| Representative | *L. vulgaris* |

### শনাক্তকারী বৈশিষ্ট্য

- Body elongated, streamlined mantle।
- Eight arms + **two long tentacles**।
- Posterior/lateral mantle-এ paired triangular/rhomboid fins।
- Internal chitinous **pen/gladius** present।
- Arms/tentacles bear suckers।

**Diagnostic character:** Elongated squid body with ten appendages, paired fins and internal pen.

### Practical identification

> Elongated mantle, 8 arms + 2 tentacles এবং paired fins-এর কারণে specimen-টি *Loligo*।


---

## 37. Octopus — *Octopus vulgaris*

**Common name:** Common octopus  
**Figure:** `museum-specimens/octopus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Mollusca |
| Class | Cephalopoda |
| Subclass | Coleoidea |
| Order | Octopoda |
| Family | Octopodidae |
| Genus | *Octopus* |
| Representative | *O. vulgaris* |

### শনাক্তকারী বৈশিষ্ট্য

- Head well developed with large eyes।
- Mouth-এর চারদিকে exactly **8 muscular arms**।
- Arms-এ numerous suckers।
- External shell absent; no long feeding tentacle pair।
- Muscular mantle ও funnel present।

**Diagnostic character:** Eight sucker-bearing arms and no external shell.

### Practical identification

> Eight sucker-bearing arms এবং shell-এর অনুপস্থিতির কারণে specimen-টি *Octopus*।


---

## 38. Eupagurus — *Eupagurus* (traditional) — commonly *Pagurus* in modern usage

**Common name:** Hermit crab  
**Figure:** `museum-specimens/eupagurus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Arthropoda |
| Subphylum | Crustacea / Pancrustacea |
| Class | Malacostraca |
| Order | Decapoda |
| Infraorder | Anomura |
| Family | Paguridae |
| Modern representative genus | *Pagurus* |

### শনাক্তকারী বৈশিষ্ট্য

- Cephalothorax heavily calcified; abdomen soft, curved ও asymmetrical।
- Soft abdomen সাধারণত empty gastropod shell-এর ভিতরে থাকে।
- First pereiopods chelate; chelae often unequal।
- Abdominal appendages asymmetrically reduced।
- Uropods abdomen-কে shell-এর ভিতরে anchor করতে সাহায্য করে।

**Diagnostic character:** Soft asymmetrical coiled abdomen adapted to occupation of a gastropod shell.

### Practical identification

> Soft asymmetrical abdomen এবং borrowed gastropod shell ব্যবহারের কারণে specimen-টি hermit crab—traditional *Eupagurus*।

**Taxonomy / syllabus note:** WoRMS-এ বহু *Eupagurus* species modern *Pagurus* বা অন্য pagurid genera-তে accepted।


---

## 39. Limulus — *Limulus polyphemus*

**Common name:** Horseshoe crab  
**Figure:** `museum-specimens/limulus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Arthropoda |
| Subphylum | Chelicerata |
| Traditional class | Merostomata |
| Order | Xiphosura |
| Family | Limulidae |
| Genus | *Limulus* |
| Representative | *L. polyphemus* |

### শনাক্তকারী বৈশিষ্ট্য

- Broad horseshoe-shaped **prosoma** with hard dorsal shield।
- Posterior opisthosoma followed by long pointed **telson**।
- Antennae absent; chelicerae present।
- Ventral opisthosoma-তে book gills।
- Lateral compound eyes visible dorsally।

**Diagnostic character:** Horseshoe-shaped prosomal shield + long sword-like telson.

### Practical identification

> Horseshoe-shaped carapace ও long telson-এর কারণে specimen-টি *Limulus*।


---

## 40. Millipedes — Millipede — representative *Julus* / Class Diplopoda

**Common name:** Millipede  
**Figure:** `museum-specimens/millipedes.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Arthropoda |
| Subphylum | Myriapoda |
| Class | Diplopoda |
| Representative order | Julida |
| Representative genus | *Julus* |

### শনাক্তকারী বৈশিষ্ট্য

- Body usually cylindrical ও elongated।
- Most apparent trunk **diplosegments**-এ two pairs of legs।
- Head-এ one pair antennae।
- First trunk segment forms **collum**।
- Defensive glands (repugnatorial glands) many segments-এ থাকতে পারে।

**Diagnostic character:** Two pairs of legs on most diplosegments.

### Practical identification

> Most trunk diplosegments-এ two pairs of legs থাকার কারণে specimen-টি Millipede, Class **Diplopoda**।


---

## 41. Centipedes — Centipede — representative *Scolopendra*

**Common name:** Centipede  
**Figure:** `museum-specimens/centipedes.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Arthropoda |
| Subphylum | Myriapoda |
| Class | Chilopoda |
| Order | Scolopendromorpha |
| Family | Scolopendridae |
| Representative genus | *Scolopendra* |

### শনাক্তকারী বৈশিষ্ট্য

- Body dorsoventrally flattened; head + long trunk।
- Each typical trunk segment-এ one pair of legs।
- Head-এ one pair long antennae।
- First trunk appendages modified into venom-bearing **forcipules**।
- Fast terrestrial predator।

**Diagnostic character:** One pair legs per segment + venomous forcipules.

### Practical identification

> One pair legs per trunk segment এবং forcipules থাকার কারণে specimen-টি Centipede, Class **Chilopoda**।


---

## 42. Palaemon — *Palaemon* sp. (representative *P. serratus*)

**Common name:** Prawn  
**Figure:** `museum-specimens/palaemon.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Arthropoda |
| Subphylum | Crustacea / Pancrustacea |
| Class | Malacostraca |
| Order | Decapoda |
| Infraorder | Caridea |
| Family | Palaemonidae |
| Genus | *Palaemon* |

### শনাক্তকারী বৈশিষ্ট্য

- Body cephalothorax ও segmented abdomen-এ divided।
- Cephalothorax carapace-covered এবং anteriorly **rostrum**।
- Five pairs pereiopods; anterior pairs chelate হতে পারে।
- Two pairs antennae এবং stalked compound eyes।
- Pleopods abdominally; telson + uropods form tail fan।

**Diagnostic character:** Caridean prawn with rostrum, five walking-leg pairs and tail fan.

### Practical identification

> Rostrum, decapod pereiopods এবং tail fan-এর কারণে specimen-টি *Palaemon*।


---

## 43. Antedon — *Antedon bifida*

**Common name:** Feather star  
**Figure:** `museum-specimens/antedon.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Echinodermata |
| Subphylum | Pelmatozoa |
| Class | Crinoidea |
| Subclass | Articulata |
| Order | Comatulida |
| Family | Antedonidae |
| Genus | *Antedon* |
| Representative | *A. bifida* |

### শনাক্তকারী বৈশিষ্ট্য

- Central disc থেকে five primary arms arise করে, usually bifurcate হয়ে 10 arms produce।
- Arms-এ numerous lateral **pinnules**, feather-like appearance দেয়।
- Adult free-moving but aboral **cirri** দিয়ে temporarily attach করতে পারে।
- Mouth এবং anus oral/upward surface-এ।
- Adult permanent stalk absent।

**Diagnostic character:** Feather-like pinnulate arms and aboral cirri; no adult stalk.

### Practical identification

> Pinnulate feather-like arms এবং aboral cirri-এর কারণে specimen-টি *Antedon*।


---

## 44. Asterias — *Asterias rubens*

**Common name:** Common starfish  
**Figure:** `museum-specimens/asterias.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Echinodermata |
| Class | Asteroidea |
| Clade | Forcipulatacea |
| Order | Forcipulatida |
| Family | Asteriidae |
| Genus | *Asterias* |
| Representative | *A. rubens* |

### শনাক্তকারী বৈশিষ্ট্য

- Central disc থেকে usually five broad arms/rays।
- Arms disc-এর সঙ্গে broadly continuous।
- Oral side-এ open **ambulacral grooves** ও sucker-bearing tube feet।
- Aboral surface-এ spines, papulae এবং pedicellariae।
- Madreporite aboral side-এ visible।

**Diagnostic character:** Broad five-rayed star with open ambulacral grooves and sucker-bearing tube feet.

### Practical identification

> Five broad arms, open ambulacral grooves এবং sucker-bearing tube feet-এর কারণে specimen-টি *Asterias*।


---

## 45. Echinus — *Echinus esculentus*

**Common name:** Sea urchin  
**Figure:** `museum-specimens/echinus.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Echinodermata |
| Class | Echinoidea |
| Order | Camarodonta |
| Family | Echinidae |
| Genus | *Echinus* |
| Representative | *E. esculentus* |

### শনাক্তকারী বৈশিষ্ট্য

- Body globular; arms absent।
- Fused calcareous plates rigid **test** তৈরি করে।
- Test-এ many movable spines।
- Five ambulacral + five interambulacral zones।
- Oral side-এ Aristotle's lantern।

**Diagnostic character:** Globular rigid test with movable spines and five ambulacral zones.

### Practical identification

> Globular test, movable spines এবং five ambulacral zones-এর কারণে specimen-টি *Echinus*।


---

## 46. Holothuria — *Holothuria* sp. (e.g., *H. atra*)

**Common name:** Sea cucumber  
**Figure:** `museum-specimens/holothuria.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Echinodermata |
| Class | Holothuroidea |
| Order | Holothuriida |
| Family | Holothuriidae |
| Genus | *Holothuria* |

### শনাক্তকারী বৈশিষ্ট্য

- Body cucumber-shaped, elongated ও leathery।
- Rigid external test absent; skeleton reduced to microscopic ossicles।
- Mouth-এর চারদিকে oral tentacles।
- Ventral tube feet often more developed।
- Internal respiratory trees present।

**Diagnostic character:** Soft cucumber-shaped echinoderm with oral tentacles and reduced ossicles.

### Practical identification

> Cucumber-shaped soft body ও oral tentacles-এর কারণে specimen-টি *Holothuria*।


---

## 47. Ophiura — *Ophiura ophiura*

**Common name:** Brittle star  
**Figure:** `museum-specimens/ophiura.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Echinodermata |
| Class | Ophiuroidea |
| Order-level clade | Amphilepidida / Ophiurina |
| Family | Ophiuridae |
| Genus | *Ophiura* |
| Representative | *O. ophiura* |

### শনাক্তকারী বৈশিষ্ট্য

- Central disc sharply demarcated from arms।
- Five long, slender, highly flexible arms।
- Arms articulated by vertebral ossicles and move mainly sideways।
- Ambulacral grooves closed; tube feet generally without terminal suckers।
- Anus absent in typical ophiuroids।

**Diagnostic character:** Clearly distinct disc with long slender jointed brittle-star arms.

### Practical identification

> Distinct central disc এবং long flexible articulated arms-এর কারণে specimen-টি *Ophiura*।


---

## 48. Cucumaria — *Cucumaria frondosa*

**Common name:** Dendrochirotid sea cucumber  
**Figure:** `museum-specimens/cucumaria.svg`

### Classification

| Rank | Taxon |
|---|---|
| Kingdom | Animalia |
| Phylum | Echinodermata |
| Class | Holothuroidea |
| Clade | Dendrochirotacea |
| Order | Dendrochirotida |
| Family | Cucumariidae |
| Genus | *Cucumaria* |
| Representative | *C. frondosa* |

### শনাক্তকারী বৈশিষ্ট্য

- Body short/stout, cucumber-like ও leathery।
- Mouth-এর চারদিকে **10 branched dendritic tentacles**।
- Tube feet often arranged in five longitudinal rows।
- Body wall-এ calcareous ossicles।
- Respiratory trees internal।

**Diagnostic character:** Sea cucumber with conspicuous branched dendritic oral tentacles.

### Practical identification

> Branched oral tentacles এবং stout cucumber-like body-এর কারণে specimen-টি *Cucumaria*।


---

# High-yield Comparison Table

| Pair | Fast distinction |
|---|---|
| *Metaphire/Pheretima* vs *Lumbricus* | perichaetine/anterior clitellum vs fewer chaetae/posterior clitellum |
| *Hirudo* vs *Piscicola* | broad medicinal leech vs narrow fish leech with conspicuous posterior sucker |
| *Fasciola* vs *Schistosoma* | leaf-shaped hermaphrodite vs dioecious, male gynecophoral canal |
| *Wuchereria* vs *Loa* microfilaria | tail-tip without nuclei vs nuclei extend to tail tip |
| *Sepia* vs *Loligo* vs *Octopus* | cuttlebone + broad fin / pen + squid fins / 8 arms only |
| Millipede vs Centipede | 2 pairs legs per diplosegment vs 1 pair per segment + forcipules |
| *Asterias* vs *Ophiura* | broad rays continuous with disc vs sharply distinct disc + slender arms |
| *Holothuria* vs *Cucumaria* | general sea cucumber vs conspicuous branched dendritic oral tentacles |

# Syllabus Nomenclature Notes

- **Metaphere:** source spelling preserved; probable *Metaphire* but independent specimen-label verification required.
- **Pheretima:** separate syllabus label retained; traditional *P. posthuma* is commonly treated today as *Metaphire posthuma*.
- **Diphyllobothrium:** syllabus label retained; broad fish tapeworm is now commonly *Dibothriocephalus latus*.
- **Hymenolepis nana:** syllabus/CDC usage retained; NCBI lists *Rodentolepis nana* as current name.
- **Convoluta:** traditional flatworm placement differs from modern Xenacoelomorpha/Acoela.
- **Sipunculus:** traditional Phylum Sipuncula remains useful for exam alignment; modern phylogeny nests sipunculans within Annelida.
- **Eupagurus:** traditional hermit-crab genus label; many species now placed in *Pagurus* or other pagurid genera.

# Spotting Template

```text
Specimen:
Phylum:
Class:
Order/Family (if required):

Identifying characters:
1.
2.
3.

Diagnostic character:

Conclusion:
উপরের diagnostic characters-এর ভিত্তিতে specimen-টি __________ হিসেবে শনাক্ত করা হলো।
```