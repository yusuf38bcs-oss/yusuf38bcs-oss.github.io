---
layout: single
author_profile: true
author: "MD. Yusuf"
sidebar:
  nav: "synaptic_nav"
title: "উদ্ভিদবিজ্ঞান: HSC Biology 1st Paper Botany"
excerpt: "উচ্চ মাধ্যমিক জীববিজ্ঞান ১ম পত্রের উদ্ভিদবিজ্ঞান শেখার পরিচ্ছন্ন অধ্যায় ও লেকচার সূচি।"
date: 2026-06-09T05:00:00.000Z
last_modified_at: 2026-07-11T16:45:00.000+06:00
permalink: /biology/hsc-corner/botany/
node_id: index-hsc-botany
pillar: "Life Sciences"
difficulty: "Intermediate"
xp: 500
time_min: 20
status: "Active"
parent_node: hsc-corner
network:
  - hsc-corner
  - botany
language: bn
curriculum_tracks:
  - HSC Botany
hsc_alignment: "HSC Biology 1st Paper: Botany learning gateway"
concept_level: "Gateway"
---

<div class="lbfl-botany-index">

<section class="lbfl-clean-hero">
  <span class="lbfl-clean-kicker">HSC Biology 1st Paper · Botany</span>
  <h2>উদ্ভিদবিজ্ঞান: অধ্যায় ও লেকচার সূচি</h2>
  <p>এখানে HSC Botany শেখার প্রয়োজনীয় অধ্যায় ও সক্রিয় লেকচারগুলো সাজানো হয়েছে। প্রতিটি সক্রিয় লিংক শিক্ষার্থীকে সরাসরি সংশ্লিষ্ট পাঠে নিয়ে যাবে।</p>
</section>

<h2>অধ্যায় সূচি</h2>

<div class="lbfl-clean-grid">
  <article class="lbfl-clean-card">
    <span class="lbfl-tag">অধ্যায় ০১</span>
    <h3><a href="/biology/hsc-corner/botany/chapter-01-cell-and-its-structure/">কোষ ও এর গঠন</a></h3>
    <p>কোষ, প্রোটোপ্লাজম, কোষঝিল্লি, সাইটোপ্লাজম, রাইবোসোম ও কোষীয় অঙ্গাণুর ভিত্তিমূলক পাঠপথ।</p>
  </article>
  <article class="lbfl-clean-card">
    <span class="lbfl-tag">অধ্যায় ০২</span>
    <h3>কোষ বিভাজন</h3>
    <p>মাইটোসিস, মিয়োসিস, crossing over ও chromosome behaviour — পরবর্তী ধাপে যুক্ত হবে।</p>
  </article>
</div>

<h2>সক্রিয় লেকচার</h2>

<div class="lbfl-clean-grid">
  <article class="lbfl-clean-card">
    <span class="lbfl-tag">লেকচার ০১</span>
    <h3><a href="/bn/biology/hsc-corner/botany/lecture-01-cell-protoplasm-cell-theory/">কোষ, প্রোটোপ্লাজম ও কোষতত্ত্ব</a></h3>
    <p>কোষের মৌলিক ধারণা, প্রোটোপ্লাজম, সাইটোপ্লাজম, নিউক্লিয়াস, cell theory, MCQ ও CQ অনুশীলন।</p>
  </article>
  <article class="lbfl-clean-card">
    <span class="lbfl-tag">লেকচার ০২</span>
    <h3><a href="/biology/hsc-corner/botany/lecture-02-plasma-membrane-fluid-mosaic-model/">প্লাজমা মেমব্রেন ও Fluid Mosaic Model</a></h3>
    <p>Phospholipid bilayer, membrane protein, glycocalyx, cholesterol ও transport ধারণা।</p>
  </article>
  <article class="lbfl-clean-card">
    <span class="lbfl-tag">লেকচার ০৩</span>
    <h3><a href="/biology/hsc-corner/botany/lecture-03-cytoplasm-ribosome-protein-factory/">সাইটোপ্লাজম, রাইবোসোম ও প্রোটিন কারখানা</a></h3>
    <p>সাইটোপ্লাজম, সাইটোসোল, ribosome, 70S/80S এবং protein synthesis।</p>
  </article>
  <article class="lbfl-clean-card">
    <span class="lbfl-tag">লেকচার ০৬</span>
    <h3><a href="/bn/biology/hsc-corner/botany/lecture-06-mitochondria/">মাইটোকন্ড্রিয়া: কোষের শক্তিঘর</a></h3>
    <p>Mitochondrion structure, cristae, matrix, mitochondrial DNA, ATP production ও high-energy cell।</p>
  </article>
</div>

<h2>পরবর্তী লেকচার</h2>

<div class="lbfl-table-wrap">
<table>
  <thead>
    <tr><th>লেকচার</th><th>বিষয়</th></tr>
  </thead>
  <tbody>
    <tr><td>০৪</td><td>Endoplasmic Reticulum</td></tr>
    <tr><td>০৫</td><td>Golgi Body, Lysosome, Peroxisome</td></tr>
    <tr><td>০৭</td><td>Plastid and Chloroplast</td></tr>
    <tr><td>০৮</td><td>Nucleus and Chromosome</td></tr>
  </tbody>
</table>
</div>

</div>
