...
- canonical production Worker ingress is `https://api.learningbiologyforlife.org`; when the direct `https://lbfl-socratic-ai.yusuf-38bcs.workers.dev` diagnostic ingress is checked, it must report the same Worker version ID and must not receive the production certification credential;
