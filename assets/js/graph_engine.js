...
(function(){
  ...
  function GraphEngine(canvasId){
    ...
    this.nodes=[];this.edges=[];this.frame=null;this.tick=0;this.max=this.mobile?80:140;
  ...
  GraphEngine.prototype.simulate=function(){
    var self=this;if(!this.nodes.length)return;var k=.6*Math.sqrt((this.w*this.h)/(this.nodes.length||1)),byId=new Map(this.nodes.map(function(n){return[n.id,n];}));
    function step(){self.tick++;for(var i=0;i<self.nodes.length;i++){for(var j=i+1;j<self.nodes.length;j++){var a=self.nodes[i],b=self.nodes[j],dx=a.x-b.x||.01,dy=a.y-b.y||.01,d=Math.max(8,Math.sqrt(dx*dx+dy*dy)),f=(k*k)/d*.005;a.vx+=dx/d*f;a.vy+=dy/d*f;b.vx-=dx/d*f;b.vy-=dy/d*f;}}
      ...
      self.nodes.forEach(function(n){n.vx*=.82;n.vy*=.82;n.x=Math.min(self.w-24,Math.max(24,n.x+n.vx));n.y=Math.min(self.h-24,Math.max(24,n.y+n.vy));});self.draw();if(self.tick<self.max)self.frame=requestAnimationFrame(step);}
  ...
  document.addEventListener('DOMContentLoaded',function(){var canvas=document.getElementById('interactive-canvas-element')||document.getElementById('neural-network');if(canvas&&!window.GraphRenderer)window.GraphRenderer=new GraphEngine(canvas.id);});
})();
