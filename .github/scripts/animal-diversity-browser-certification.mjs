...
for(const route of routes){for(const vp of viewports){
 ...
 metrics=await page.evaluate(()=>({h1:document.querySelectorAll("h1").length,cycle:!!document.querySelector("[data-zoology-learning-cycle]"),framework:!!document.querySelector(".lbfl-framework-links"),overflow:Math.max(0,document.documentElement.scrollWidth-window.innerWidth),brokenImages:Array.from(document.images).filter(i=>!i.complete||i.naturalWidth===0).map(i=>i.src),switcher:Array.from(document.querySelectorAll(".lbfl-language-switcher a")).map(a=>a.getAttribute("href")),nav:!!document.querySelector(".lbfl-course-pagination")}));
 ...
 const isLecture=/lecture-\d\d-/.test(route); const passed=status===200&&metrics.h1===1&&!metrics.cycle&&!metrics.framework&&(metrics.overflow??999)<=2&&(metrics.brokenImages?.length??1)===0&&consoleErrors.length===0&&pageErrors.length===0&&httpErrors.length===0&&violations.length===0&&(!isLecture||metrics.nav===true)&&((metrics.switcher?.length??0)>=2);
 results.push({route,viewport:vp.name,status,metrics,violations,consoleErrors,pageErrors,httpErrors,passed}); await context.close();
}}
