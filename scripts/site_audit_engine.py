...
Purpose:
- Scan the repository file-by-file.
- Detect Jekyll route, front matter, language, bilingual-pair, Markdown, safety and security risks.
- Produce Markdown and JSON reports under audit-reports/.

This script is read-only. It does not modify public site files.
"""
...
BAD_TRANSLATION_PHRASES = [
    "Native Bangla Gateway",
    "structured academic hub",
    "learning framework",
    "stale fallback",
    "Ban বোতাম",
    "Deployment refresh",
    "Official website for Learning Biology For Life",
    ".github.io",
]
...
PLACEHOLDER_PATTERNS = [
    ...
    r"Insert Your",
    r"coming soon",
    r"No items found for this archive",
]
...
SECRET_PATTERNS = [
    ...
    r"(?i)GOOGLE_CLIENT_SECRET\s*[:=]\s*['\"]?[^'\"\s]+",
...
def add_finding(report: AuditReport, severity: str, code: str, path: str, message: str, line: Optional[int] = None, evidence: Optional[str] = None) -> None:
    report.findings.append(Finding(severity=severity, code=code, path=path, message=message, line=line, evidence=evidence))
...
def is_public_content_file(path: str) -> bool:
    if path in PUBLIC_ROOT_CONTENT_FILES:
        return True
...
def compute_route_pairs(report: AuditReport) -> None:
    ...
    for route, page in by_route.items():
        ...
        elif lang == "en" and route.startswith("/en/"):
...
def write_reports(report: AuditReport, output_dir: Path) -> Tuple[Path, Path]:
    ...
    lines: List[str] = []
    lines.append("# Learning Biology For Life — Site Audit Report")
    lines.append("")
    ...
    lines.append(f"- Content files scanned: **{report.content_files_scanned}**")
    lines.append(f"- Pages/routes detected: **{len([p for p in report.pages if p.route])}**")
    lines.append(f"- Critical findings: **{counts.get('critical', 0)}**")
    lines.append(f"- High findings: **{counts.get('high', 0)}**")
    lines.append(f"- Medium findings: **{counts.get('medium', 0)}**")
    lines.append("")

    if report.missing_required_routes:
        lines.append("## Missing Required Routes")
        lines.append("")
        for route in report.missing_required_routes:
    ...
    lines.append("")
    lines.append("| Route | Lang | Title | Source | Words |")
    lines.append("|---|---:|---|---|---:|")
    ...
    if not report.findings:
        ...
        lines.append("| Severity | Code | Source | Line | Message | Evidence |")
        lines.append("|---|---|---|---:|---|---|")
...
def main() -> int:
    ...
    fail_severities = thresholds[args.fail_on]
